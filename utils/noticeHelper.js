...
const getNoticeStatus = (booking) => {
  if (
    ...
    return {
      ...
      daysRemaining: null,
      expireDate: null,
      noticeSubmittedDate: null,
    };
  }
  const noticeDate = new Date(booking.noticeSubmittedDate);
  const periodDays =
    Number(booking.noticePeriodDays) > 0
      ? Number(booking.noticePeriodDays)
      : 30;
  const expireDate = new Date(
    noticeDate.getTime() + periodDays * 24 * 60 * 60 * 1000,
  );
...
const hasActiveNotice = (spaceId, bookings) => {
  const active = Array.isArray(bookings)
    ? bookings.find(
        (b) =>
          b.spaceId === spaceId &&
          b.status === "Notice Given" &&
          b.noticeSubmittedDate,
      )
    : null;
  if (!active) {
    return {
